...
int main()
{
    ...
    events.HookEvent(GlobalFrameSetup) = std::function([&](uint32_t frame_) {
        frame = frame_;
    });
...
#ifdef BUILD_CLIENT
    Render render(
        {.log = log->CreateChild("[RENDER]"), .globalEvents = events});
#endif
    ...
    while (running) {
        ...
        events.CallEvent(GlobalFrame, frame++);
