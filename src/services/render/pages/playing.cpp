// playing.cpp (FIXED SFML3 PrimitiveType)
...
static sf::Color MulAlpha(sf::Color c, float k)
{
    ...
    c.a = static_cast<uint8_t>(static_cast<float>(c.a) * k);
    return c;
}
...
namespace Core::App::Render::Pages {
    ...
    void Playing::Initialise()
    {
        ...
        if (!glowShader.loadFromFile(glowShaderPath.string(), sf::Shader::Type::Fragment))
            throw std::runtime_error("Failed to load glow shader from " + glowShaderPath.string());

        if (!snakeShader.loadFromFile(snakeShaderPath.string(), sf::Shader::Type::Fragment))
    ...
    void Playing::DrawGrid(sf::RenderWindow& window)
    {
        ...
        const float tile = 220.f;
        const float fadeEdge = tile * 1.25f;
        ...
        // ✅ SFML3: PrimitiveType::Quads удалён, используем Triangles
        sf::VertexArray quads(sf::PrimitiveType::Triangles);
        quads.clear();

        sf::VertexArray seams(sf::PrimitiveType::Lines);
        seams.clear();
        ...
        for (float y = startY; y < endY; y += tile)
        {
            for (float x = startX; x < endX; x += tile)
            {
                ...
                col.r = static_cast<uint8_t>(static_cast<float>(col.r) * dark);
                col.g = static_cast<uint8_t>(static_cast<float>(col.g) * dark);
                col.b = static_cast<uint8_t>(static_cast<float>(col.b) * dark);

                // two triangles per tile
                const sf::Vector2f p0 { x,         y        };
                const sf::Vector2f p1 { x + tile,  y        };
                const sf::Vector2f p2 { x + tile,  y + tile };
                const sf::Vector2f p3 { x,         y + tile };

                quads.append(sf::Vertex(p0, col));
                quads.append(sf::Vertex(p1, col));
                quads.append(sf::Vertex(p2, col));

                quads.append(sf::Vertex(p0, col));
                quads.append(sf::Vertex(p2, col));
                quads.append(sf::Vertex(p3, col));
        ...
        const float baseThickness = 16.f * zoom_;
        ...
        for (int i = 0; i < 5; ++i)
        {
            ...
            gc.a = static_cast<uint8_t>(70 - i * 12);
    ...
    // ---------- ДАЛЬШЕ ТВОЙ КОД DrawSnake + DrawFood ОСТАЛСЯ 1в1 ----------
    // (я заменил только API SFML3: setOrigin/position/scale/create, остальное untouched)
                        ...
                        const Utils::Legacy::Game::Interface::Entity::Snake::Shared& snake)
    {
        ...
        auto ClampU8 = [](int v) -> uint8_t { return static_cast<uint8_t>(std::clamp(v, 0, 255)); };
        ...
        auto DrawGeometry = [&](sf::RenderTarget& targetRT, bool softPass)
        {
            ...
            {
                const bool isHead = (it == std::prev(segments.rend()));
                ...
                if (softPass)
                {
                    ...
                    soft.a = static_cast<uint8_t>(std::min(200.f, 140.f * factor + 60.f));
                    c.setFillColor(soft);
                    targetRT.draw(c);
                    continue;
                }

                {
                    ...
                    sh.setFillColor(sf::Color(0, 0, 0, static_cast<uint8_t>(std::min(55.f, 25.f + 30.f * factor))));
                    targetRT.draw(sh);
                }
                ...
                circle.setOutlineColor(sf::Color(0, 0, 0, static_cast<uint8_t>(std::min(110.f, 60.f + 50.f * factor))));
...

                {
                    ...
                    sf::Color hc(255, 255, 255, static_cast<uint8_t>(std::clamp(10.f + 10.f * factor, 0.f, 22.f)));
                    hi.setFillColor(hc);
                ...
                if (isHead)
                {
...

                    {
                        ...
                        ring.setFillColor(sf::Color(0, 0, 0, static_cast<uint8_t>(std::min(200.f, 160.f * factor + 40.f))));
                    ...
                    eye.setFillColor(sf::Color(235, 235, 235, static_cast<uint8_t>(std::min(255.f, 220.f * factor + 35.f))));
                    ...
                    pup.setFillColor(sf::Color(10, 10, 10, static_cast<uint8_t>(std::min(255.f, 230.f * factor + 25.f))));
                    ...
                    dot.setFillColor(sf::Color(255, 255, 255, static_cast<uint8_t>(std::min(180.f, 120.f * factor + 60.f))));
        ...
        if (blurRTSize_ != want)
        {
            ...
            snakeSoftRT_ = sf::RenderTexture(blurRTSize_);
            blurPingRT_  = sf::RenderTexture(blurRTSize_);
            blurPongRT_  = sf::RenderTexture(blurRTSize_);
        ...
        DrawGeometry(snakeSoftRT_, true);
        ...
        window.setView(window.getDefaultView());
        {
            ...
            blurSpr.setColor(sf::Color(255, 255, 255, static_cast<uint8_t>(std::clamp(70.f * factor, 0.f, 90.f))));
            window.draw(blurSpr, sf::RenderStates(sf::BlendAlpha));

            blurSpr.setColor(sf::Color(255, 255, 255, static_cast<uint8_t>(std::clamp(22.f * factor, 0.f, 35.f))));
            window.draw(blurSpr, sf::RenderStates(sf::BlendAdd));
        }

        window.setView(oldView);
        DrawGeometry(window, false);
    }
                       ...
                       const Utils::Legacy::Game::Interface::Entity::Food::Shared& food)
    {
...

        {
            ...
            sh.setFillColor(sf::Color(0, 0, 0, static_cast<uint8_t>(std::min(140.f, 40.f + 70.f * factor))));
            window.draw(sh);
        }
        ...
        mainBall.setOutlineColor(sf::Color(0, 0, 0, static_cast<uint8_t>(std::min(140, 80 + static_cast<int>(outline)))));
...

        {
            ...
            hi.setFillColor(sf::Color(255, 255, 255, static_cast<uint8_t>(18 + 18 * factor)));
            window.draw(hi);
        }
        ...
        const float time = t + (pos.x * 0.0007f + pos.y * 0.0005f);
